...
Arquitectura en tres capas: presentación en Vercel (Astro 6 `output: 'server'`, i18n ES/EN, View Transitions), lógica en AWS (`sloWorkApi` con GraphQL, Sequelize, waitlist/welcome) y datos en RDS.

`slowork-landing-v2` concentra home, blog (posts vía GraphQL + `marked`), formularios BFF (`POST /api/waitlist/`, `/api/contact/`) con validación Zod y GA4 consolidado. `sloWorkFront` es el portal Angular 21 con Apollo Client, `signalStore` y módulos de blog, sites y admin. `slowork-content-creators` es una landing Astro estática con Content Layer, JSON-LD y proxy serverless para solicitudes de creadores.
